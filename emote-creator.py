...
def draw_emote(body_color, body_style,
    ...
    def star(center, radius, color):
        ...
        for i in range(5):
            ...
            pts.append((cx_ + radius*math.cos(angle), cy_ - radius*math.sin(angle)))
            angle = math.radians(126 + i*72)
            pts.append((cx_ + radius*0.4*math.cos(angle), cy_ - radius*0.4*math.sin(angle)))
    ...
    for idx, (ex, ey) in enumerate(positions):
        ...
        if eye_shape == 'Open':
            ...
            for i in range(4):
                draw.arc(
                    [(ex-i*eye_r*0.1, ey-i*eye_r*0.1), (ex+i*eye_r*0.1, ey+i*eye_r*0.1)],
                    start=0, end=270, fill=eye_color, width=1
                )
    ...
    if mouth_style == 'Smile':
        draw.arc([(mx-mw, my-mh), (mx+mw, my+mh)], start=0, end=180, fill=mouth_color, width=t)
    elif mouth_style == 'Frown':
        draw.arc([(mx-mw, my-mh), (mx+mw, my+mh)], start=180, end=360, fill=mouth_color, width=t)
        ...
        for i in range(1,5):
            draw.line(
                [(mx-mw+i*mw*0.4, my-mh*0.2), (mx-mw+i*mw*0.4, my+mh*0.2)],
                fill=mouth_color, width=int(t/2)
            )
    elif mouth_style == 'Oops':
        draw.arc([(mx-mw, my-mh), (mx+mw, my+mh)], start=90, end=270, fill=mouth_color, width=t)
    ...
    for ex, ey in positions:
        ...
        if brow_style == 'Straight':
            ...
            draw.arc([(ex-brow_len, yb-brow_len), (ex+brow_len, yb+brow_len)], start=0, end=180, fill=brow_color, width=t)
        elif brow_style == 'Sad':
            draw.arc([(ex-brow_len, yb-brow_len), (ex+brow_len, yb+brow_len)], start=180, end=360, fill=brow_color, width=t)
            ...
            draw.arc([(ex-brow_len, yb-brow_len), (ex+brow_len, yb+brow_len)], start=0, end=360, fill=brow_color, width=t)
...
thickness = st.sidebar.slider('Line thickness', 1, 20, 10, key='thickness')
body_color = st.sidebar.color_picker('Body color', '#2AFF00', key='body_color')
body_style = st.sidebar.selectbox('Body style', ['Filled','Outline'], index=1, key='body_style')

st.sidebar.subheader('Eyes')
eye_color = st.sidebar.color_picker('Eye color', '#2AFF00', key='eye_color')
eye_style = st.sidebar.selectbox('Eye style',['Filled','Outline'], index=0, key='eye_style')
eye_shape = st.sidebar.selectbox('Eye shape', ['Open','Closed','Wink','Happy','Sad','Surprised','Sleepy','Angry','Excited','Dazed'], index=0, key='eye_shape')

st.sidebar.subheader('Mouth')
mouth_color = st.sidebar.color_picker('Mouth color', '#2AFF00', key='mouth_color')
mouth_style = st.sidebar.selectbox('Mouth type',['Smile','Frown','Neutral','Surprised','Tongue','Laugh','Sad','OpenSmile','Grimace','Oops'], index=0, key='mouth_style')

st.sidebar.subheader('Eyebrows')
brow_color = st.sidebar.color_picker('Brow color', '#2AFF00', key='brow_color')
brow_style = st.sidebar.selectbox('Eyebrow style',['None','Straight','Angled','Raised','Sad','Frown','Rounded','Zigzag'], index=0, key='brow_style')

# Randomizer button
if st.sidebar.button('Randomize'):
    # pick random colors
    for key in ['body_color','eye_color','mouth_color','brow_color']:
        st.session_state[key] = '#%06x' % random.randint(0, 0xFFFFFF)
    # pick random styles
...
if formats:
    ...
    for fmt,col in zip(formats,cols):
        if fmt=='PNG':
            ...
            svg = generate_svg({})
            col.download_button('SVG', svg, file_name='emote.svg', mime='image/svg+xml')
